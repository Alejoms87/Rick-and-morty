...
async function getAllLocationData(page) {

fetch(`https://rickandmortyapi.com/api/location?page=${page}`).then(response => response.json())
...
// // Llamada inicial para cargar la página 1

// getAllLocationData(currentPage);  (--- ACA ME ROMPE EL CODIGO!!! ---)

// Agrega controles de paginación (por ejemplo, botones "Anterior" y "Siguiente") para permitir al usuario navegar por las páginas
function previousPage() {
  if (currentPage > 1) {
    currentPage--;
    getAllLocationData(currentPage);
  }
}
